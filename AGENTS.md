...
- **Dashboard** — role-based panels (`app/[locale]/dashboard/page.tsx` branches on `user.role`; data in `server/actions/dashboard.ts`, UI in `components/dashboard/`): dentist → next-hour queue + attended-today history (most-recent-first) + give-turn links, auto-refreshing every 60s (`use-panel-refresh.ts`, paused when tab hidden); receptionist → today + rest of week, follow-ups (late / no-show / not-completed, derived from status+time), give-turn links, unpaid + recent payments, same auto-refresh; admin → the original 4-KPI cards (expand later). Panels reuse `AttendSheet` and `GenerateTurnLinkDialog`; clinic wall-clock times are computed server-side so cards never depend on browser TZ.
