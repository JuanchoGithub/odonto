...
- **Appointment outcomes** — statuses `scheduled | arrived | in_chair | completed | cancelled | no_show` (terminal: completed/cancelled/no_show; reopening one requires the explicit `reopen=true` ack surfaced in the dialog). **Reprogramming** = any starts/ends/dentist change in `updateAppointment` — auto-bumps `reprogram_count`, sets `original_starts_at` once, and writes an `audit_log` `reschedule` row with from/to; rendered as a "Reprogramado ×N" chip (list cards, patient page) and a ↺N marker on calendar blocks. **Cancellation** is manual with a picklist `cancel_reason` (patient_request / dentist_request / no_answer / duplicate / schedule_change / other) + `cancelled_at`/`cancelled_by`. **No-show** is automatic: Vercel Cron hits `GET /api/cron/mark-no-shows` daily (see `vercel.json`; guarded by `CRON_SECRET`, grace via `NO_SHOW_GRACE_MIN`, default 60 min); an overdue `scheduled` appointment flips to `no_show` (`no_show_by='system'`, the reserved system user — see `lib/system-user.ts`) unless payment/invoice/treatment/attachment/odontogram evidence in the visit window proves attendance (then auto-`completed`); `arrived`/`in_chair` never auto-flip (patient did show) — they land in the secretary "Sin completar" follow-up, which now has a "Marcar como atendido" one-tap confirm. `server/actions/appointments.ts::sweepOverdueNoShows` is the shared core; system-origin rows attribute to the reserved `users(id='system')` row (password `*LOCKED*`, login rejected) in both `appointments.no_show_by` and `audit_log.user_id`. Never store a magic string like `'auto'` in an FK column — it throws `SQLITE_CONSTRAINT_FOREIGNKEY` mid-sweep (the appointment row was already updated, so it half-applies). e2e: `e2e/appointment-outcomes.spec.ts`
- **Insurers** (obras sociales) — master table, searchable, with inline onboarding
- **Treatments** — per-patient pipeline + cost; shared `treatment_catalog` (one definitive `CONSULTA` + general entries; dentist customs enter as provisional, secretary/admin approve in Settings → Catalog)
- **Billing** — invoices (two-rate tax, **prices are final/IVA incluido**: `lib/tax.ts::splitTaxInclusive` backs out net/IVA from the typed price, clinic rates authoritative) + payments + jsPDF export; completed turnos auto-attach a `CONSULTA` treatment (`ensureConsultaForAppointment`); **Facturar visita** in `AttendSheet` builds one issued invoice per appointment (`buildVisitInvoice`, idempotent via `invoices.appointment_id` unique) with bundling (consulta + other treatment → consulta line at 0 `incluida`)
- **Search everywhere is type-ahead (store-backed, zero invocations).** `/patients` and `/insurers` show a suggestions dropdown while typing (debounced ~300ms over the local snapshot; Enter opens the highlighted match, Submit still loads the filtered list). The patient/insurer/catalog pickers (add-appointment dialog, turn-link dialog, patient form, attend sheet) read the offline-first store (`lib/store/options.ts`) instead of hitting `/api/*` per keystroke or per dialog open. Shared pieces: `lib/hooks/use-server-search.ts` (debounced, abortable, stale-safe), `components/ui/search-suggest.tsx` (dropdown, accepts a local `fetchItems` override), `lib/patient-options.ts` (store-backed patient fetcher). The `/api/patients`, `/api/insurers`, `/api/catalog` GET routes remain for direct use (tests, deep links) but no UI picker calls them in steady state.
- **Reports** — recharts dashboards (revenue, top treatments, no-show, by-dentist)
- **Settings** — clinic profile (currency, locale, tax rates) + user management + treatment catalog (definitive/provisional)
- **Attachments** — Vercel Blob storage (X-rays, photos, consent)
- **Dashboard** — role-based panels (`app/[locale]/dashboard/page.tsx` branches on `user.role`; projections computed client-side in `lib/store/projections.ts`, UI in `components/dashboard/`): dentist → next-hour queue + attended-today history (most-recent-first) + single add-turn entry; receptionist → today + rest of week, follow-ups (late / no-show / not-completed, derived from status+time), single add-turn entry, unpaid + recent payments; admin → the original 4-KPI cards (expand later). Panels read the offline-first store and refresh via the 5-minute delta sync (instant on tab foreground, paused when hidden) — there is no per-minute polling. `server/actions/dashboard.ts` keeps the original server queries (types reused by cards) but panels no longer call them. Panels reuse `AttendSheet` and `AddAppointmentDialog` (`GenerateTurnLinkDialog` remains for the patient-page share button); clinic wall-clock uses the SSR-passed `clinicTz` + a cached client formatter (`lib/store/time.ts`) so cards never depend on browser TZ and never build per-row server formatters.
- **Turn picker** — staff generate a signed, single-use, self-expiring link (`/pick-turn/[token]`) so patients book their own slot; single-use (consumed on booking), idle-revoked after 5 days (configurable via `TURN_PICKER_IDLE_MS`), absolute expiry default 14 days. **Agenda horizon** (`lib/agenda-horizon.ts`): bookable window is `max(today+14, auto-anchor, manual)` — from the 15th of the month the auto-anchor is the Sunday ending the first full Mon–Sun week of next month (sticky, folds back to 14 days), plus a per-dentist manual `users.agenda_open_until` set in Profile → Times (extend-only date picker, min today+15, max today+62, triple-confirm over 30 days); the ICS feed forward leg and staff date-picker max follow the same horizon, staff writes are never blocked. Links have a `purpose`: `create` (new turn) or `reprogram` (moves the existing appointment, dentist + exact duration locked — issued from the WhatsApp button's Notify/Reprogram menu, `components/ui/whatsapp-button.tsx`). While a reprogram link is active the turn keeps `status='scheduled'` plus a `reprogram_link_id` pending flag (chip "Reprogramación pendiente"); booking moves the row (`reprogram_count+1`, `audit_log reschedule via=reprogram_link`). The flag clears on consume/revoke/expiry and on any terminal flip (incl. the daily no-show sweep, which also revokes the link). e2e: `e2e/turn-reprogram.spec.ts`
...
| dentist | R/W | R/W | R/W | R/W | R + bill-visit | R/W | R | – |
| receptionist | R/W | R/W | – | – | R/W | R/W | R | – |

Dentists can build visit invoices (`billing:invoice_visit` via **Facturar visita**) and propose catalog entries (`catalog:propose`); manual invoices/payments (`billing:write`) and catalog approval (`catalog:write`) stay admin/receptionist. `can(role, action)` in `lib/rbac.ts` is the source of truth.
...
**Sync columns** (added in 0021_appointments_updated_at.sql): nullable `updated_at` + `idx_*_updated_at` on `appointments`, `invoices`, `payments`, `treatments`, `treatment_catalog` (backfilled from `created_at`/`issued_at`/`paid_at`). Every write path bumps them — this is what makes incremental delta sync correct. `patients`/`insurers`/`teeth_chart` already had `updated_at`. When adding a new write to any synced table, always set `updated_at` or the row will be invisible to other devices until a full resync.

`PRAGMA foreign_keys = ON` is set in the migration. **Never disable it.**

`createPatientInline` exists for non-redirecting flows (e.g. inline new-patient from appointment dialog). The standalone `/patients/new` form queues offline (see below) and lands on the list instead of redirecting to detail.

---

## 9b. Offline-first sync (Fluid-compute reduction)

Reads are served from a localStorage snapshot cache so steady-state views cost ~1 function invocation per 5 min per tab instead of per-poll server actions:

- **Store** (`lib/store/`): `snapshots.ts` (typed cache + `updated_at` watermarks, fingerprint snapshots for `insurers`/`catalog`, cross-tab `storage` sync, cleared on sign-out), `projections.ts` (pure client versions of the dashboard/calendar computations), `options.ts` (picker lists + relevance-ranked type-ahead), `time.ts` (cached client wall-clock), `bus.ts` (pub/sub), `sync.ts` (orchestrator), `write.ts` (queued-write builders), `mutations.ts` (write queue + conflicts), `session.ts` (`signOutAndClear`), `use-store-version.ts`.
- **Sync** (`POST /api/sync`, one invocation per run): pushes the queued mutations (validated against `lib/schemas/entities.ts`, replayed through the `*Core` functions in `server/actions/*`, per-op last-write-wins conflict detection via `baseVersion` vs server `updated_at`), then pulls `updated_at`-watermark deltas (appointments scoped to 30d past + all future) and fingerprint-checked master snapshots. Rows transfer only on change.
- **Cadence**: app-shell `<SyncLoop/>` + per-view `useAutoSync()` run one full incremental sync every 5 min while visible + instantly on tab foreground; `useEnsureSeeded()` + self-healing searches (`ensure*Seeded` in `options.ts`) cover fresh sessions. No continuous polling anywhere.
- **Writes — hybrid**: time-critical writes flush immediately (appointments, odontogram, invoices, payments, attachments, turn-picker booking — 1 invocation each). Bulk writes queue offline and flush at sync: patients (`PatientForm queueMode`, incl. inline dialogs), insurers (`InsurerForm`, inline `NewInsurerDialog`), catalog (`CatalogManager`), treatments (`PatientTreatments`). Queued creates use client-generated ids adopted by the server, so FK references (e.g. appointment → just-created patient, flushed first via `ensurePatientSynced`) never break. The `<SyncBadge/>` in `TopNav` shows the pending count and force-syncs on tap; conflicts land in a local `needsReview` log.
- **e2e pattern**: after a queued save, click the sync badge, wait for it to hide, then resolve ids via `/api/*?q=` — see `flushQueueAndOpenPatient` / `saveAndSync` / `waitForSyncIdle` in `e2e/helpers.ts`. Fresh sessions need `TURSO_URL='file:./.e2e.db'` migrated + fixtures (all fixture inserts must set `updated_at`).
...
- `components/appointments/appointment-list.tsx` — mobile = `<ul>` of compact 2-line cards (`min-h-[64px]`, `gap-2`, `px-2.5 py-2`, no color bar, no status badge). Line 1: name + bold right-aligned start time; line 2: date + reason (`EEE d MMM · reason`, truncated) + reprogram chip + WhatsApp icon on the right (`onPhoneSaved` → `onChanged` refresh, `border-0` 44×44, the only action). Terminal states tint the row instead of a badge (`statusRowTint`: completed green, cancelled red, no-show amber, each with a dark-mode variant; active states keep `bg-card`); the status stays in the card's `aria-label` and on the sheet/dialog it opens. The desktop table keeps its Status badge column. For `scheduled | arrived | in_chair` rows the whole card opens `AttendSheet` (one-tap attend, any pointer); other rows open the edit dialog directly.
- `components/appointments/attend-sheet.tsx` — the new "Attend" mode. Patient header, `tel:` call link, Edit button (`data-testid="attend-edit"`, opens the edit/reschedule dialog via the `onEdit` prop). All three mounts (`week-calendar.tsx`, `secretary-panel.tsx`, `doctor-panel.tsx`) pass `onEdit` and mount an `AppointmentDialog` for it: the secretary dialog keeps the cross-dentist picker (receptionists reschedule across dentists), the doctor dialog passes `viewerRole="dentist"` + `currentUserId` so the dentist field stays hidden/fixed to self. Status stepper (`scheduled → arrived → in_chair → completed`) one-tap advance — **self-updating via `localStatus`**: the badge (`attend-status-badge`) and the next-step button change the instant the write succeeds, without waiting for the parent's sync reconcile (parents still reconcile the open sheet so phone/context fields stay fresh). The sheet **never auto-closes**, not even at `completed` — the stepper disappears and Facturar visita stays available so the visit can be billed in the same session. Quick links to `/patients/[id]?tab=odontogram` and `?tab=treatments` still navigate away. New entries: `appointments.attend`, `appointments.attendTitle`, `appointments.advanceTo`, `appointments.openChart`, `appointments.call`.
