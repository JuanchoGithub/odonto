...
- `components/appointments/attend-sheet.tsx` — the new "Attend" mode. Patient header, `tel:` call link, Edit button (`data-testid="attend-edit"`, opens the edit/reschedule dialog via the `onEdit` prop wired to `openEdit` in `week-calendar.tsx`), status stepper (`scheduled → arrived → in_chair → completed`) one-tap advance, plus quick links to `/patients/[id]?tab=odontogram` and `?tab=treatments`. New entries: `appointments.attend`, `appointments.attendTitle`, `appointments.advanceTo`, `appointments.openChart`, `appointments.call`.
