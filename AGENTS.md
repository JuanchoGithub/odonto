...
- `components/appointments/appointment-list.tsx` — mobile = `<ul>` of cards (`min-h-[64px]`, dentist color bar, panel-style layout). Card layout matches `PanelApptCard`: name + bold right-aligned start time, a status dot + `EEE d MMM · HH:mm–HH:mm · dentist · reason` subtitle, status badge + reprogram chip, phone as a 44×44 icon button (`tel:`), and a WhatsApp icon (`onPhoneSaved` → `onChanged` refresh). For `scheduled | arrived | in_chair` rows on **touch (coarse pointer)** the whole card opens `AttendSheet` (one-tap attend) and a 44×44 pencil button (`data-testid="appt-edit"`, aria-label `appointments.edit`) keeps the edit/reschedule dialog reachable; on non-coarse pointers the card opens the edit dialog and a labeled **Attend** button (`data-testid="appt-attend"`, `min-h-[48px]`) opens `AttendSheet`.
