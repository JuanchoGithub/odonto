...
- `components/appointments/appointment-list.tsx` — mobile = `<ul>` of compact 3-line cards (`min-h-[64px]`, `gap-2`, `px-2.5 py-2`, no color bar). Line 1: name + bold right-aligned start time; line 2: date only (`EEE d MMM`) + WhatsApp icon on the right (`onPhoneSaved` → `onChanged` refresh, `border-0` 44×44); line 3: status badge + reprogram chip + reason. No `tel:` call button, no edit/attend buttons — WhatsApp is the only action. For `scheduled | arrived | in_chair` rows the whole card opens `AttendSheet` (one-tap attend, any pointer); other rows open the edit dialog directly.
